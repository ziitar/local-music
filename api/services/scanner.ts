...
import { fixEncoding } from "../utils/metadataCleaner.ts";
 ...
 * Uses ON CONFLICT to handle race conditions and ensure no duplicates
 *
...
export async function getOrCreateArtists(
  ...
  for (let i = 0; i < cleanedArtists.length; i++) {
    ...
    // Use ON CONFLICT for atomic upsert - prevents duplicates even with concurrent inserts
...
export async function getOrCreateAlbum(
  ...
  const result = await sql`
    ...
    VALUES (${cleaned}, ${metadata.trackTotal || null}, ${metadata.year || null})
